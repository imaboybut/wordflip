...
describe('기존 카드 간격 갱신', () => {
  ...
  it('Good은 ease 배율로 증가하고 ease는 유지된다', () => {
    const prev = makeSchedule({ cardId: 'c1', intervalSteps: 100, ease: 2.3 });
    const result = rateCard(prev, 'c1', 'good', 50);
    expect(result.intervalSteps).toBe(230);
    expect(result.ease).toBe(2.3);
  });

  it('Easy는 가장 크게 증가한다 (I * ease * 1.6)', () => {
    const prev = makeSchedule({ cardId: 'c1', intervalSteps: 100, ease: 2.3 });
    const result = rateCard(prev, 'c1', 'easy', 50);
    expect(result.intervalSteps).toBe(368);
...
describe('손상 상태 fallback (Again 3 / Hard 12 / Good 40 / Easy 120)', () => {
  const corrupt = [
  ...
  it.each(corrupt)('%s → 안전 기본값 사용', (_label, prev) => {
    ...
    expect(rateCard(prev, 'c1', 'good', 50).intervalSteps).toBe(40);
    expect(rateCard(prev, 'c1', 'easy', 50).intervalSteps).toBe(120);
...
describe('결과 불변식', () => {
  ...
  it('소수 계산 결과는 정수로 반올림된다', () => {
    const prev = makeSchedule({ cardId: 'c1', intervalSteps: 41, ease: 2.31 });
    const result = rateCard(prev, 'c1', 'good', 50);
    expect(Number.isInteger(result.intervalSteps)).toBe(true);
    expect(result.intervalSteps).toBe(Math.round(41 * 2.31));
  });
