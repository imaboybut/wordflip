...
- 날짜 기반이 아닌 **넘김 횟수(step) 기반** 간격 반복: 하루 제한도, "오늘 학습 완료" 화면도 없음
...
| 평가 | 앞면의 `Good(알아요)` 한 버튼. 카드를 누르면 자동 `모름`+뜻 공개, 한 번 더 누르면 다음 카드 |
| 스케줄러 | 전역 `studyStep` 카운터 기반. 날짜/시각은 계산에 사용하지 않음 |
...
날짜와 시각은 절대 사용하지 않습니다. 카드를 평가할 때마다 전역 정수
`studyStep`이 1 증가하고, 각 카드는 `dueStep <= studyStep`이 되면 다시
나타납니다.

- 사용자 동작: `카드 탭(모름) +3 / Good(알아요) 최소 +800` step
- 기존 카드(간격 `I`, 계수 `ease`, 초기 2.30):
  - `Again(모름)`: 이전 간격과 관계없이 `+3`, ease −0.20, lapse +1
  - `Hard`: `max(12, I×1.2)`, ease −0.05
  - `Good(알아요)`: 일반 카드는 `max(800, I×ease)`
  - 직전 평가가 `Again`인 카드는 누적 오답에 따라 `+40~200`으로 한 차례
    재확인하고, 다음 연속 `Good`에서 최소 `+800`으로 졸업
  - `Easy`: `max(2400, I×ease×1.6)`, ease +0.15
- ease는 `[1.30, 3.20]`, 간격은 `[3, 20000]` step으로 clamp
- 계산 결과가 비정상(NaN/∞/음수)이면 손상으로 간주하고 안전 기본값(3/12/800/2400)을 사용

현재 학습 화면에서는 `Again`이 카드 탭에 자동 연결되고 `Good`만 버튼으로
표시됩니다. `Hard/Easy` 값은 기존 로그·백업 호환을 위해 내부적으로 유지합니다.

카드 선택 규칙: due 카드 우선 → 복습 3연속이면 신규 1장 → 최근 5장 반복 회피 →
신규 소진 시 가장 오래 안 본 카드. due 카드 중에서는 직전에도 틀린 카드와 누적
오답이 많은 카드에 제한된 우선 보너스를 주되, 아주 오래 밀린 카드는 결국
추월하도록 해 다른 복습이 굶지 않게 합니다. 같은 카드는 즉시 연속으로 나오지
않습니다(카드가 1장뿐인 경우 제외).
