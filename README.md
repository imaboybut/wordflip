...
- 약 8,000개 단어 내장 (필수 회화 표현 + 회화 어휘 + 학술/AI·CS/전문 어휘)
...
| 평가 | `Again(다시) / Hard(어려움) / Good(알겠음) / Easy(쉬움)` 버튼 + 스와이프(← Again, ↓ Hard, ↑ Good, → Easy) |
| 스케줄러 | 전역 `studyStep` 카운터 기반. 날짜/시각은 계산에 사용하지 않음 |
| 발음 | 기기 내장 음성(`speechSynthesis`, en-US 우선) + 한글 발음 표기 |
...
- 신규 카드: `Again +3 / Hard +12 / Good +40 / Easy +120` step
- 기존 카드(간격 `I`, 계수 `ease`, 초기 2.30):
  - `Again`: `max(3, I×0.25)`, ease −0.20, lapse +1
  - `Hard`: `max(12, I×1.2)`, ease −0.05
  - `Good`: `max(40, I×ease)`
  - `Easy`: `max(120, I×ease×1.6)`, ease +0.15
- ease는 `[1.30, 3.20]`, 간격은 `[3, 20000]` step으로 clamp
- 계산 결과가 비정상(NaN/∞/음수)이면 손상으로 간주하고 안전 기본값(3/12/40/120)을 사용

카드 선택 규칙: due 카드 우선 → 복습 3연속이면 신규 1장 → 최근 5장 반복 회피 →
신규 소진 시 가장 오래 안 본 카드. 같은 카드는 즉시 연속으로 나오지 않습니다
(카드가 1장뿐인 경우 제외).

## 단어 데이터

- `public/data/words.csv` — 약 8,000행. 스키마:
...
- iOS의 `speechSynthesis` 음성 목록은 첫 재생 후에 채워질 수 있고, 음질은
  기기에 설치된 시스템 음성에 따라 다릅니다.
